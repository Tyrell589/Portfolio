const ExperienceData = [
  {
    title: "Junior Web Developer",
    company: "Skyline IT Services",
    duration: "Jul 2018 – Aug 2020",
    description: [
      "Built user-friendly front-end components using HTML, CSS, JavaScript, and jQuery.",
      "Supported bug fixes and feature improvements in live systems.",
      "Gained hands-on experience in teamwork, version control (Git), and client communication."
    ],
  },
  {
    title: "Full-Stack Developer",
    company: "BrightWave Digital Ltd",
    duration: "Sep 2020 – Dec 2022",
    description: [
      "Built and maintained modern web platforms with responsive UI/UX using React.js and Next.js.",
      "Automated internal processes, reducing manual work for clients by 30%.",
      "Collaborated with cross-functional teams to deliver tailored software solutions on schedule."
    ],
  },
  {
    title: "Senior Full-Stack Developer",
    company: "TechNova Solutions Ltd",
    duration: "Jan 2023 – Jun 2025",
    description: [
      "Led the development of scalable web applications using Node.js, React.js, and MySQL/MongoDB.",
      "Designed and implemented API integrations to streamline client operations and save development time.",
      "Mentored junior developers and reviewed code to ensure best practices and quality standards."
    ],
  }
];

export default ExperienceData;
